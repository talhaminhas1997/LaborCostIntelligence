import { motion } from "framer-motion";
import { cn, usdK } from "@/lib/utils";
import type { FlagKind, Job } from "@/lib/types";

const KIND_LABEL: Record<FlagKind, string> = {
  "added-scope": "Added scope",
  rework: "Rework",
  underbid: "Underbid",
  "under-recovery": "Under-recovery",
};

export function PortfolioRail({
  flagged,
  monitoring,
  calm,
  activeJobId,
  resolvedJobs,
  onSelect,
}: {
  flagged: Job[];
  monitoring: Job[];
  calm: Job[];
  activeJobId: string | null;
  resolvedJobs: Set<string>;
  onSelect: (job: Job) => void;
}) {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-ink-200 px-4 py-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-ink-800">Portfolio</h3>
          <span className="text-xs text-ink-400">
            {flagged.length + monitoring.length + calm.length} active jobs
          </span>
        </div>
      </div>

      <div className="scroll-thin flex-1 overflow-y-auto px-3 py-3">
        {/* Flagged — the few that matter, ranked */}
        <SectionLabel
          tone="danger"
          title="Needs you"
          sub="ranked by margin at risk"
        />
        <div className="space-y-2">
          {flagged.map((job, i) => {
            const resolved = resolvedJobs.has(job.id);
            return (
              <motion.button
                key={job.id}
                initial={{ opacity: 0, x: 8 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.06 }}
                onClick={() => onSelect(job)}
                className={cn(
                  "w-full rounded-lg border p-3 text-left transition-all",
                  activeJobId === job.id
                    ? "border-brand-400 bg-brand-50 shadow-sm"
                    : resolved
                    ? "border-emerald-200 bg-emerald-50/60 hover:border-emerald-300"
                    : "border-ink-200 bg-white hover:border-brand-300"
                )}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className="flex h-4 w-4 items-center justify-center rounded bg-ink-900 text-[10px] font-semibold text-white">
                      {job.flag!.rank}
                    </span>
                    <span className="text-xs font-semibold text-ink-800">
                      Job {job.number}
                    </span>
                  </div>
                  <span
                    className={cn(
                      "rounded-full px-2 py-0.5 text-[10px] font-medium",
                      resolved
                        ? "bg-emerald-100 text-emerald-700"
                        : "bg-rose-100 text-rose-700"
                    )}
                  >
                    {resolved ? "Protected" : KIND_LABEL[job.flag!.kind]}
                  </span>
                </div>
                <p className="mt-1 truncate text-xs text-ink-500">{job.name}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="font-mono text-[10px] text-ink-400">
                    {job.flag!.costCode}
                  </span>
                  <span
                    className={cn(
                      "tabular text-xs font-semibold",
                      resolved ? "text-emerald-600" : "text-rose-600"
                    )}
                  >
                    {resolved
                      ? `+${job.flag!.marginRecovered} pts`
                      : `${usdK(job.flag!.marginAtRisk)} at risk`}
                  </span>
                </div>
              </motion.button>
            );
          })}
        </div>

        {/* Monitoring — drifting but deliberately not surfaced */}
        {monitoring.length > 0 && (
          <>
            <SectionLabel
              tone="warn"
              title="Watching"
              sub="drifting · below threshold"
            />
            <div className="space-y-1">
              {monitoring.map((job) => (
                <button
                  key={job.id}
                  onClick={() => onSelect(job)}
                  className={cn(
                    "flex w-full items-center justify-between rounded-md px-3 py-2 text-left transition-colors",
                    activeJobId === job.id ? "bg-amber-50" : "hover:bg-amber-50/60"
                  )}
                >
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400" />
                    <span className="truncate text-xs text-ink-600">
                      <span className="font-medium text-ink-700">
                        Job {job.number}
                      </span>{" "}
                      · {job.name}
                    </span>
                  </div>
                  <span className="tabular shrink-0 text-[11px] text-ink-400">
                    {Math.round(job.pctComplete * 100)}%
                  </span>
                </button>
              ))}
            </div>
          </>
        )}

        {/* Calm — most of the portfolio, on budget */}
        <SectionLabel
          tone="success"
          title="On budget"
          sub="no action needed"
        />
        <div className="space-y-1">
          {calm.map((job) => (
            <button
              key={job.id}
              onClick={() => onSelect(job)}
              className={cn(
                "flex w-full items-center justify-between rounded-md px-3 py-2 text-left transition-colors",
                activeJobId === job.id ? "bg-ink-100" : "hover:bg-ink-100/70"
              )}
            >
              <div className="flex min-w-0 items-center gap-2">
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-400" />
                <span className="truncate text-xs text-ink-600">
                  <span className="font-medium text-ink-700">
                    Job {job.number}
                  </span>{" "}
                  · {job.name}
                </span>
              </div>
              <span className="tabular shrink-0 text-[11px] text-ink-400">
                {Math.round(job.pctComplete * 100)}%
              </span>
            </button>
          ))}
        </div>
      </div>
    </div>
  );
}

function SectionLabel({
  tone,
  title,
  sub,
}: {
  tone: "danger" | "warn" | "success";
  title: string;
  sub: string;
}) {
  const color =
    tone === "danger"
      ? "text-rose-600"
      : tone === "warn"
      ? "text-amber-600"
      : "text-emerald-600";
  return (
    <div className="mb-2 mt-5 flex items-center gap-2 px-1 first:mt-0">
      <span className={cn("text-[11px] font-semibold uppercase tracking-wide", color)}>
        {title}
      </span>
      <span className="text-[11px] text-ink-400">· {sub}</span>
    </div>
  );
}
