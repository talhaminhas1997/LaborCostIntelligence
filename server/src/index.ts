...
import { computeAnalyze, computeChat, computeExtract } from "./fallbacks.js";
import type {
  AnalyzeRequest,
  AnalyzeResult,
  ChatRequest,
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, model: getClient() ? "live" : "fallback" });
});
...
app.post("/api/chat", async (req, res) => {
  ...
  if (!client || !Array.isArray(body.messages)) {
    return res.json({ reply: computeChat(body), source: "fallback" });
  }
  try {
    ...
    res.json({ reply: computeChat(body), source: "fallback" });
  }
});
...
app.post("/api/extract", async (req, res) => {
  ...
  if (!client) {
    return res.json({ ...computeExtract(body), source: "fallback" });
  }
  ...
  try {
    ...
    res.json({ ...computeExtract(body), source: "fallback" });
  }
});
...
app.post("/api/analyze", async (req, res) => {
  ...
  const client = getClient();
  if (!client) return res.json(computeAnalyze(request));
...
Analyze and return ONLY the JSON.`;
    ...
    const jsonStr = extractJson(textOf(message));
    if (!jsonStr) throw new Error("no JSON");
    res.json(normalizeAnalyze(JSON.parse(jsonStr), request));
  } catch (err) {
    console.error("[analyze] fallback:", (err as Error).message);
    res.json(computeAnalyze(request));
  }
});

app.listen(PORT, () => {
  const mode = getClient() ? "live Claude" : "deterministic fallback";
